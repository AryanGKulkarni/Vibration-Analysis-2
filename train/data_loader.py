...
def load_data():
    ...
    unbalace_data = pd.read_csv(unbalance_csv)
    ...
    normal_data['label'] = 0
    unbalace_data['label'] = 1
    misalignment_data['label'] = 2
    misalignment_with_unbalance_data['label'] = 3
    
    # Combine all data into a single DataFrame
    combined_data = pd.concat([normal_data, unbalace_data, misalignment_data, misalignment_with_unbalance_data], ignore_index=True)
